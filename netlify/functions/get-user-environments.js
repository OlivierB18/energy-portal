...
const fetchUserConfig = async (domain, token, userId) => {
  const response = await fetch(
    `https://${domain}/api/v2/users/${encodeURIComponent(userId)}?fields=app_metadata,user_metadata&include_fields=true`,
    {
      headers: { Authorization: `Bearer ${token}` },
    },
  )
  ...
  return {
    environmentIds: Array.isArray(user.app_metadata?.environmentIds)
      ...
      : [],
    userMetadata: user.user_metadata && typeof user.user_metadata === 'object'
      ? user.user_metadata
      : {},
  }
}
...
export const handler = async (event) => {
  try {
    ...
    const requestUserId = String(event.queryStringParameters?.userId || '').trim()
    const requesterUserId = String(payload.sub || '').trim()
    const targetUserId = requestUserId || requesterUserId

    if (!targetUserId) {
      return { statusCode: 401, body: JSON.stringify({ error: 'Invalid token' }) }
    }

    if (requestUserId && requestUserId !== requesterUserId) {
    ...
    const managementToken = await getManagementToken(domain)
    const { environmentIds, userMetadata } = await fetchUserConfig(domain, managementToken, targetUserId)
